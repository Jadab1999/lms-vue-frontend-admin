            ...
            <form @submit.prevent="userPortalLogin('submit')" class="user_portal_login">
              <div class="p-fluid">
                <div class="p-field">
                  ...
                  <InputText type="mobile" v-model="loginValue.mobile" id="mobile" />
    ...
    setup() {
      ...
      const userPortalLogin = async(location) => {
        ...
        if(loginValue.isSubmitted && location === 'submit'){
          try{
            const response = await axios.post( labourLoginAuth, {
              mobile: loginValue.mobile,
            ...
            if(response.status === 200) {
              ...
              router.push({name: "LaboursDashboard"});
